...
import { FindByUuidPerson } from './find-by-uuid-person';
...
describe('FindByUuidPerson', () => {
  let useCase: FindByUuidPerson;
  ...
  beforeEach(async () => {
    ...
    const useCases: Provider[] = [FindByUuidPerson];

    const module: TestingModule = await Test.createTestingModule({
      providers: [...infrastructure, ...useCases],
    }).compile();

    useCase = module.get<FindByUuidPerson>(FindByUuidPerson);
  });
  ...
  describe('findByUuid', () => {
    it('should findByUuid Person', async () => {
      ...
      expect(mockPersonRepository.findOneByUuid).toHaveBeenCalledWith(uuid);
